...
import { EventPurchase } from '../../../domain/events/entities/EventPurchase';
...
import { RegisterEventPurchaseInputDTO, EventResponseDTO } from '../dtos/EventDTOs';
import { EventMapper } from '../mappers/EventMapper';

export class RegisterEventPurchase implements IUseCase<RegisterEventPurchaseInputDTO, EventResponseDTO> {
  constructor(
    private readonly eventRepository: IEventRepository,
  ) {}

  async execute(input: RegisterEventPurchaseInputDTO): Promise<Result<EventResponseDTO>> {
    try {
      ...
      // Create purchase detail entity (for validation purposes)
      EventPurchase.create({
        eventId: savedEvent.id,
        supplierId: new UniqueId(input.supplierId),
        purchasePrice: input.purchasePrice,
        purchaseWeight: input.purchaseWeight ? Weight.create(input.purchaseWeight) : undefined,
      ...
      return Result.fail<EventResponseDTO>(
        error instanceof Error ? error.message : 'Failed to register purchase event',
      );
    }
  }
}
