...
interface GetGMAAnimalsInputDTO {
  readonly gmaId: string;
}

export class GetGMAAnimals implements IUseCase<GetGMAAnimalsInputDTO, GMAResponseDTO> {
  constructor(
    private readonly gmaRepository: IGMARepository,
  ) {}

  async execute(input: GetGMAAnimalsInputDTO): Promise<Result<GMAResponseDTO>> {
    try {
      ...
      const message = error instanceof Error ? error.message : 'Unexpected error retrieving GMA animals';
